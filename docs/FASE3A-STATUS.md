...
- r16: a confirmação chegou **duplicada** — o Decide da Kapso reprocessa "a última mensagem" a cada passagem (espera/poll + inbound), e cada `/internal/turns` com o mesmo `#reset` refazia o reset e reenviava. Idempotência por mensagem via `sdr.webhook_receipts` (chave `reset:<tenant>:<marca>:<id>`): um comando reseta e confirma uma vez só. Marcador como mensagem `system` foi descartado porque os montadores de contexto (Sofia, briefing, lab) leem todos os atores.
- Efeito colateral da duplicação (antes da r16): o segundo reset apagou a linha da primeira confirmação; o WAMID dela voltou pelo histórico como "humano assumiu" (eventos `handoff` 06:53:44/45 sem execução), a conversa ficou `human` e o "oi" seguinte não gerou job; execução Kapso `26bad572` encerrada às 07:00Z. A API religa (`resume`) ao ver uma execução nova numa conversa `human`.
- Deploy r16 (`sprint5-reset-once-20260916-r16`, commit `1671137`) às 06:59:59Z, 25/25 na imagem, `/health`/`/ready` 200. Rollback: r15.
- r17: `#reset` como **primeira mensagem de uma conversa nova** na Kapso retornava antes de criar a linha da conversa → confirmação não gravada → replay como humano → handoff (conversa `3d7c1ea2`, 07:02Z). O reset agora roda depois de criar candidato/conversa. Deploy `sprint5-reset-first-20260916-r17` (commit `962e9de`) às 07:09:01Z, 24/24 na imagem, `/health`/`/ready` 200. Rollback: r16.
