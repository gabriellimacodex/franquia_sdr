...
6. Após `handoff`, a execução Kapso fica em `waiting` no nó de espera; "resume" no inbox reabre a espera mas o status nativo continua `handoff`, e `sapore-session` re-encaminha (`native_not_running`) por desenho. Para testar de novo: encerrar a execução (`PATCH workflow_executions/{id}` `status: ended`) ou usar outro número.
