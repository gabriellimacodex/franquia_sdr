...
export class Kapso {
 ...
 async request(path:string,init:RequestInit={}) {
  const response=await this.transport('https://api.kapso.ai'+path,{...init,headers:{'X-API-Key':this.config.KAPSO_API_KEY,'Content-Type':'application/json'},signal:AbortSignal.timeout(5000)});
